...
class PheromoneStrategy(AntStrategy):
    ...
    def decide_action(self, perception: AntPerception) -> AntAction:
        ...
        if self.current_memory.is_searching:
            ...
            dir = self.get_max_pheromone_direction(perception)
            if dir == None:
                return self.update(self.random_move(), perception)
            if dir.value == Direction.get_left(perception.direction).value:
                return self.update(AntAction.TURN_LEFT, perception)
            elif dir.value == perception.direction.value:
                return self.update(AntAction.MOVE_FORWARD, perception)
            elif dir.value == Direction.get_right(perception.direction).value:
                return self.update(AntAction.TURN_RIGHT, perception)
            else:
                return self.update(self.random_move(), perception)
            
        
        elif perception.has_food:
            if not self.current_memory.is_last_pheromone and self.current_memory.last_move not in [AntAction.TURN_LEFT, AntAction.TURN_RIGHT]:
                self.current_memory.is_last_pheromone = True
    ...
    def get_max_pheromone_direction(self, perception: AntPerception):
        max_strength = 0
        best_dir = None

        for (dx, dy), strength in perception.food_pheromone.items():
            if strength > max_strength:
                max_strength = strength
                best_dir = self.get_dir(dx, dy)
